...
def quick_sort(a_list, start, end):
    ...
    pivot = partitionRandomFunction(a_list, start, end)
...
# First Function
def partitionMiddleFunction(a_list, start, end):
...
# Second Function
def partitionEndFunction(a_list, start, end):
    a_list[start], a_list[end] = a_list[end], a_list[start]
    return partition(a_list, start, end)

# Third Function
def partitionMedianFunction(a_list, start, end):
    middle = len(a_list) // 2
    average = (middle + start + end) // 3
    a_list[start], a_list[average] = a_list[average], a_list[start]
    return partition(a_list, start, end)

# Fourth Function Randomly chooses a pivot.
def partitionRandomFunction(a_list, start, end):
